...
### Docker（推荐，内嵌 PostgreSQL）

```bash
git clone https://github.com/1Panel-dev/AI-Portal.git
cp .env.example .env
# 编辑 .env 配置 PANEL_BASE_URL 和 PANEL_API_KEY（连接 1Panel 网关）
docker-compose up -d
```

访问 `http://localhost:3000`

> 内嵌 PostgreSQL，无需单独部署数据库。数据持久化在 `./data` 目录中。
...
- **技能市场** — 浏览、搜索、安装 AI 技能（Skill），配套 CLI 工具 `skillctl`
...
├── docs/          文档
├── Dockerfile     all-in-one 版（内嵌 PostgreSQL）
...
| 变量 | 说明 |
|------|------|
| `INIT_ADMIN_PASSWORD` | 首次启动创建 admin 账号（不设置则无管理员） |
