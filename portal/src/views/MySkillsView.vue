  ...
  <div>
    ...
    <main class="max-w-[960px] mx-auto px-6 py-[92px] pb-20">
      <div class="flex items-end justify-between gap-4 mb-7">
        ...
        <router-link v-if="featureFlags.skillSubmitEnabled && canSubmit" to="/submit" class="px-4 py-2 text-sm font-medium bg-accent text-white rounded-lg hover:bg-accent-hover transition-colors no-underline">
          提交技能
        </router-link>
      </div>
      ...
      <div v-else-if="skills.length === 0" class="bg-white border border-[rgba(0,0,0,0.04)] rounded-xl p-10 text-center shadow-card">
        ...
        <router-link v-if="featureFlags.skillSubmitEnabled && canSubmit" to="/submit" class="inline-flex px-4 py-2 text-sm font-medium bg-accent text-white rounded-lg hover:bg-accent-hover transition-colors no-underline">
          去提交
        </router-link>
      </div>
