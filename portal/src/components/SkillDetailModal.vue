              ...
              <li>
                ...
                <div class="mt-1.5 space-y-1">
                  <div class="relative group">
                    <code class="block text-[12px] font-mono text-text bg-surface-secondary rounded-lg px-3 py-1.5 pr-8 truncate">{{ loginCommand }}</code>
                    <button
                      @click="copyLoginCommandToClipboard"
                      class="absolute right-1.5 top-1/2 -translate-y-1/2 p-1 rounded opacity-0 group-hover:opacity-100 transition-opacity text-text-tertiary hover:text-text"
...
async function checkAuth(url) {
  ...
  try {
    ...
    const filename = `${(props.skill?.slug || 'skill').replace(/^1panel-/, '')}.zip`
...
const loadVersions = async () => {
  ...
  if (versionsSlug.value === props.skill.slug) return
  versionsTimer = setTimeout(() => { versionsLoading.value = true }, 200)
  try {
    ...
    versionsSlug.value = props.skill.slug
  } catch {
    versions.value = []
  } finally {
